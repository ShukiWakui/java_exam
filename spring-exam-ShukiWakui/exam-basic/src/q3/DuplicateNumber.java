...
public class DuplicateNumber {
    ...
    public static void main(String[] args) {
        List<Integer> firstNumList = new LinkedList<>();
        List<Integer> secondNumList = new LinkedList<>();
        for (int num : NUMBERS) {
            if (!firstNumList.contains(num)) {
                firstNumList.add(num);
            } else if (!secondNumList.contains(num)) {
                secondNumList.add(num);
            }
        }
        for (int n : secondNumList) {
            System.out.print(n + " ");
        }
